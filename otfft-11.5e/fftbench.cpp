...
double laptime(int LOOPS, int TRIES, const FFT& fft, complex_t *x)
{
...
int main(int argc, char *argv[]) try
{
    ...
    cout << "------+-----------------+-----------------+---\n";
    cout << "length| SimpleFFT [usec]|   OTFFT   [usec]|err\n";
    cout << "------+-----------------+-----------------+---\n";
    ...
    for (int n = n_min; n <= n_max; n++) {
        ...
        initialize(N, x1);
        //OTFFT::speedup_magic();
        lap = laptime(LOOPS, TRIES, SimpleFFT::FFT(N), x1);
        sum1 += lap*n/n_max;
        cout << ff(11,2, lap/LOOPS) << "(" << ff(3,0, 100) << "%)|" << flush;
        msleep(DELAY2);
        lap1 = lap;
        ...
        initialize(N, x1);
        //OTFFT::speedup_magic();
        lap = laptime(LOOPS, TRIES, OTFFT::FFT(N), x1);
        sum2 += lap*n/n_max;
    ...
    cout << "------+-----------------+-----------------+---\n";
    cout << " cost |";
    cout << ff(11,2, sum1/(n_max-n_min+1)) << "(" << ff(3,0, 100) << "%)|" << flush;
    cout << ff(11,2, sum2/(n_max-n_min+1)) << "(" << ff(3,0, 100*sum2/sum1) << "%)|" << endl;
    cout << "------+-----------------+-----------------+---\n";
